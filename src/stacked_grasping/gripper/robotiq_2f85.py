...
ROBOTIQ_2F85_BACKEND = "robotiq-2f85"
ROBOTIQ_2F85_BODY_NAME = "robotiq_2f85"
ROBOTIQ_2F85_ROOT_JOINT_NAME = "robotiq_2f85_freejoint"
ROBOTIQ_2F85_LEFT_DRIVER_JOINT_NAME = "left_driver_joint"
ROBOTIQ_2F85_RIGHT_DRIVER_JOINT_NAME = "right_driver_joint"
ROBOTIQ_2F85_XML_ENV = "SO_GRASP_ROBOTIQ_2F85_XML"

_PROJECT_ROBOTIQ_2F85_XML = Path("mujoco_menagerie") / "robotiq_2f85" / "2f85.xml"
...
def resolve_robotiq_2f85_xml(
    ...
    *,
    ...
    if explicit_path is not None:
        ...
        env_path = values.get(ROBOTIQ_2F85_XML_ENV)
        if env_path:
            candidates.append(Path(env_path))
        base = Path(root) if root is not None else project_root()
        candidates.append(base / _PROJECT_ROBOTIQ_2F85_XML)

    missing_asset_messages: list[str] = []
    for candidate in candidates:
        if not candidate.exists():
            continue
        missing_assets = _missing_required_assets(candidate)
        if not missing_assets:
            return candidate
        missing_asset_messages.append(f"{candidate}: missing {', '.join(missing_assets)}")

    searched = "\n  ".join(str(path) for path in candidates)
    if missing_asset_messages:
        searched += "\n  " + "\n  ".join(missing_asset_messages)
    raise FileNotFoundError(
        "No complete Robotiq 2F-85 MuJoCo XML found. "
        f"Pass --robotiq-2f85-xml or set {ROBOTIQ_2F85_XML_ENV}.\nSearched:\n  {searched}"
    )


def robotiq_2f85_config_from_graspnet_candidate(
    grasp: GraspPoseCandidate,
    *,
    include_freejoint: bool = True,
    grasp_center_local: tuple[float, float, float] = (0.0, 0.0, 0.1558),
) -> Robotiq2F85Config:
...
def append_robotiq_2f85_to_mujoco_root(
    root: ET.Element,
    *,
    gripper_xml_path: str | Path,
    output_path: str | Path | None,
    config: Robotiq2F85Config | None = None,
) -> None:
    cfg = config or Robotiq2F85Config()
    source_path = Path(gripper_xml_path)
    ...
    _append_robotiq_body(root, source_root, cfg)
    _append_reference_sections(root, source_root)


def _missing_required_assets(xml_path: Path) -> list[str]:
    assets_dir = xml_path.parent / "assets"
    return [name for name in _REQUIRED_ASSET_FILES if not (assets_dir / name).exists()]
...
def _append_robotiq_body(root: ET.Element, source_root: ET.Element, config: Robotiq2F85Config) -> None:
    ...
    if source_worldbody is None:
        raise ValueError("Robotiq 2F-85 XML does not contain <worldbody>.")

    source_body = source_worldbody.find("body[@name='base_mount']")
    if source_body is None:
        source_body = source_worldbody.find("body")
    if source_body is None:
        raise ValueError("Robotiq 2F-85 XML does not contain a gripper body.")

    gripper_body = copy.deepcopy(source_body)
    _rename_body_references(gripper_body, {"base_mount": ROBOTIQ_2F85_BODY_NAME})
    gripper_body.attrib["name"] = ROBOTIQ_2F85_BODY_NAME
    gripper_body.attrib["pos"] = _fmt_vec(config.pos)
    gripper_body.attrib["quat"] = _fmt_vec(config.quat)
    if config.include_freejoint:
        _ensure_freejoint(gripper_body)
    worldbody.append(gripper_body)
...
def _append_reference_sections(root: ET.Element, source_root: ET.Element) -> None:
    for tag in ("contact", "tendon", "equality", "actuator"):
        ...
        for child in list(source_section):
            copied = copy.deepcopy(child)
            _rename_body_references(copied, {"base_mount": ROBOTIQ_2F85_BODY_NAME})
            destination_section.append(copied)


def _ensure_freejoint(gripper_body: ET.Element) -> None:
    existing = gripper_body.find("freejoint")
    if existing is not None:
        existing.attrib["name"] = ROBOTIQ_2F85_ROOT_JOINT_NAME
        return
    gripper_body.insert(0, ET.Element("freejoint", {"name": ROBOTIQ_2F85_ROOT_JOINT_NAME}))
